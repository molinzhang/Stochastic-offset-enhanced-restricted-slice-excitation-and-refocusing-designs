...
from adpulses_sgd import metrics, penalties
...
def m2p(m2pName: str,
    ...
    *OUTPUTS*:
    ...
    cube1 = SpinCube(shape, fov, **cube_d, **dkw)

    # %% cube and its preprocessing
    tmp = f_st2dic(mfile['cube2_st'])
    shape, fov = ((1,)+tuple(tmp['dim'].flatten()),
                  f_tensor(tmp['fov'].flatten()))
    cube_d = {k: f_tensor(tmp[k]) for k in ('T1', 'T2', 'M','loc')}
    cube_d['mask'] = f_tensor(tmp['mask'], dtype=torch.bool)
    ...
    cube2 = SpinCube(shape, fov, **cube_d, **dkw)
    ...
    target['d1_'] = cube1.extract(f_tensor(tmp['d1']),None,  torch.tensor([1]))  # (1,nM,xyz)
    target['weight1_'] = cube1.extract(f_tensor(tmp['weight1'].astype(np.float)))
    
    target['d2_'] = cube2.extract(f_tensor(tmp['d2']),None,  torch.tensor([1]))  # (1,nM,xyz)
    target['weight2_'] = cube2.extract(f_tensor(tmp['weight2'].astype(np.float)))
    ...
    return target, cube, cube1, cube2, pulse, arg

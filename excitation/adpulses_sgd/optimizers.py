...
import mrphy_sgd
from mrphy_sgd.mobjs import SpinCube, Pulse
import torch

import numpy as np
import random

from adpulses_sgd.penalties import pen_l2_mat, pen_inf_mat, rf_smooth, crush_stop
from adpulses_sgd.metrics import err_stdmxy
...
def arctanLBFGS(
    ...
    tρ, θ = mrphy_sgd.utils.rf2tρθ(pulse.rf, pulse.rfmax)
    tsl_nr = mrphy_sgd.utils.s2ts(mrphy_sgd.utils.g2s(pulse.nr, pulse.dt), pulse.smax)  #This is used for lineara gradient. g2s computes the slew rate. s2ts computes tan() to avoid contraints

    #tsl_nr_cons = mrphy_sgd.utils.s2ts(mrphy_sgd.utils.g2s(torch.unsqueeze(pulse.nr[:,:,0],-1), pulse.dt), pulse.smax)

    tsl_lr = mrphy_sgd.utils.s2ts(mrphy_sgd.utils.g2s(pulse.lr, pulse.dt), pulse.smax)
    ...
    opt_rf = optim.LBFGS([tρ, θ], lr=0.00005, max_iter=10, history_size=100, # 5# 0.005 # 0.0005 is too small
                         tolerance_change=1e-4,
                         line_search_fn='strong_wolfe') # lr = 3 ## 4mm lr = 1 / 1 # lr = 0.01

    opt_sl = optim.LBFGS([tsl_nr], lr=0.005, max_iter=20, history_size=100, # 1
                         tolerance_change=1e-6,
                         line_search_fn='strong_wolfe') # lr = 0.1, 20? ## 4mm lr = 0.5 / 0.1 # lr = 0.005
    
    scheduler_rf = optim.lr_scheduler.MultiStepLR(opt_rf, milestones=[150, 170], gamma=0.1) # 200, 280
    ...
    def fn_loss(cube, pulse, idx, offset_loc_ = None):
        ...
        if idx == 1:
            loss_err, loss_pen, loss_pengr, loss_edge, loss_rfsm = fn_err(Mr_, Md_, w_=w_), fn_pen(pulse.rf), pen_inf_mat(pulse.gr), pen_l2_mat(pulse.gr), err_stdmxy(Mr_,w1_)#pen_l2(pulse.rf)
        if idx == 2:
            loss_err, loss_pen, loss_pengr, loss_edge, loss_rfsm = fn_err(Mr_, Md1_, w_=w_), fn_pen(pulse.rf), pen_inf_mat(pulse.gr), pen_l2_mat(pulse.gr), err_stdmxy(Mr_,w1_)#pen_l2(pulse.rf)
        if idx == 3:
            loss_err, loss_pen, loss_pengr, loss_edge, loss_rfsm = fn_err(Mr_, Md2_, w_=w2_), fn_pen(pulse.rf), pen_inf_mat(pulse.gr), pen_l2_mat(pulse.gr),pen_l2_mat(pulse.gr)#err_stdmxy(Mr_,w1_)#err_stdmxy(Mr_,w1_)#pen_l2_mat(pulse.gr)#
        if idx == 4:
            loss_err, loss_pen, loss_pengr, loss_edge, loss_rfsm = fn_err(Mr_, Md2_, w_=w1_), fn_pen(pulse.rf), pen_inf_mat(pulse.gr), pen_l2_mat(pulse.gr), pen_l2_mat(pulse.gr)#err_stdmxy(Mr_,w1_)#err_stdmxy(Mr_,w1_)#pen_l2_mat(pulse.gr)#err_stdmxy(Mr_,w1_)            
    ...
    def logger(i, t0, loss_err1, loss_err2, loss_err3, loss_pen, loss_pengr, loss_edge, loss_rfsm):
        loss = 1 * loss_err1 + 1*loss_err2 +  0.2* loss_err3 + eta*loss_pen + 0.0*loss_pengr + 0.0*loss_edge + 10.0 * loss_rfsm
    ...
    for i in range(niter):
        ...
        #print(pos_choice)
        offset_loc_f1 = torch.rand((1, 88, 88, 60, 3))#torch.ones((1, 88,88,60, 3)) * 0.5#torch.rand((1, 88, 88, 60, 3))#torch.ones((1, 88,88,60, 3)) * 0.5#torch.ones((1, 88,88,60, 3)) * 0.5 #torch.rand((1, 88, 88, 60, 3))   #torch.zeros((1, 30, 30, 20, 3)) + pos_choice * 0.002#torch.rand((1, 30, 30, 20, 3))        
        #offset_loc_f1 = torch.linspace(0,1,steps = 861)
        def closure():
            opt_rf.zero_grad()
            opt_sl.zero_grad()

            pulse.rf = mrphy_sgd.utils.tρθ2rf(tρ, θ, rfmax)
            #rf_smooth(pulse.rf)
            #pulse.gr = mrphy_sgd.utils.s2g(mrphy_sgd.utils.ts2s(tsl, smax), pulse.dt)
            pulse.lr = mrphy_sgd.utils.s2g(mrphy_sgd.utils.ts2s(tsl_lr, smax), pulse.dt)
            ...
            pulse.nr = mrphy_sgd.utils.s2g(mrphy_sgd.utils.ts2s(tsl_nr, smax), pulse.dt)
            ...
            loss = 1 * loss_err1 + eta*loss_pen + 1 *loss_pengr + 10.0*loss_edge + 0.0 * loss_rfsm #10.0 loss_rfsm #  1 *loss_pengr
        ...
        for _ in range(niter_rf):
            ...
            loss_crushx, loss_crushy = crush_mod_loss(M1, M2, w1_)
            loss_err3 = loss_crushx + loss_crushy  
            ...
            savemat(os.path.join('/home/molin/shimm_nick/decompose_refocus/demo/ISMRM/compare/refocus_test/Check_nosense/pulse2/', str(off_coef)+'profileselec.mat'), {'Mx': MM1, 'My':MM2, 'mask_in':ww_, 'mask_out': ww1_, 'tar_x': mmd1_, 'tar_y': mmd2_})
            '''

            
            for idx, rf_scale in enumerate(range(1)):
              if idx > 0: 
                pulse.rf = pulse.rf * rf_scale / (rf_scale - 1)
              else:
                pulse.rf = pulse.rf * 1#rf_scale / 10
              ...
              MM1 = M1.cpu().detach().numpy()
              #MM1 = np.reshape(MM1, (88, 88, 60, 3))
              ww_ = w2_.cpu().detach().numpy()
              #ww_ = np.reshape(ww_, (88, 88, 60))
              savemat(os.path.join('/home/molin/shimm_nick/decompose_refocus/demo/ISMRM/compare/refocus_test/sagittal/pulse13rwtest/', str(off_coef)+'ex.mat'), {'Mx': MM1, 'mask_in':ww_})

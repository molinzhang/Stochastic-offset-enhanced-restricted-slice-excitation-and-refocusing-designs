function [pulse_o, optInfos] = arctanAD(target, cube, cube1, cube2, pulse_i, varargin)
...
% parse
arg.b1Map = [];
[arg.niter, arg.niter_gr, arg.niter_rf] = deal(1, 1, 0);
arg.err_meth = 'l2xy';
...
[cube_st, cube1_st, cube2_st, pulse_st] = deal(cube.asstruct(), cube1.asstruct(), cube2.asstruct(), pulse_i.asstruct());

%[cube_st, pulse_st] = deal(cube, pulse_i);

% `'-v7'` for scipy.io compatibility
save(m2pName, '-v7', 'target', 'cube_st', 'cube1_st', 'cube2_st', 'pulse_st', 'arg')

[p, ~, ~] = fileparts(mfilename('fullpath')); % .m/.py must be in the same path.
pyfile = [p, '/arctanADrw.py'];

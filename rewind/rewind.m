...
%% for the first cube
%MASK1 = load('/home/molin/shimm_nick/decompose_refocus/demo/ISMRM/compare/Mask_sagital_pregnancy_small.mat'); %mask_ex_r7_shift.mat'); %r7mask.mat'); %%MASK_12mmROI_slice.mat'); %Mask_checkslice.mat');% %r7mask.mat;%%);%Mask_pregnancy_ismrm2022.mat'); %; % %% MASK
...
Target.d2 = cat(4, zeros(size(TAR)), -TAR, 1 - TAR); % cat(4, TAR, zeros(size(TAR)), 1 - TAR)

% Mask_weight = double(MASK);
% Slice_of_interest = squeeze(Mask_weight(45,:,:)); % 19, 44
% Slice_of_interest(Slice_of_interest == 1) = 1; %1.1;
% Mask_weight(45,:,:) = Slice_of_interest;


Mask_weight = double(MASK);
Slice_of_interest = squeeze(Mask_weight(:,:,31)); % 19, 44
Slice_of_interest(Slice_of_interest == 1) = 1; %1.1; % 4
Mask_weight(:,:,31) = Slice_of_interest;

Mask_weight(Mask_weight == 1) = 1;%0.0064  %0.1;
Mask_weight(TAR == 1) = 0;
...
%load('/home/molin/shimm_nick/decompose_refocus/demo/ISMRM/compare/ex_t2gradient04.mat')
%load('/home/molin/shimm_nick/decompose_refocus/demo/ISMRM/compare/refocus_test/selective/excitation/trash/pulse26.mat');
%load('/home/molin/shimm_nick/decompose_refocus/demo/ISMRM/compare/refocus_test/selective/excitation/formrm1/pulse1_rfpeak.mat')
%RF1 = load('../../rf_185.mat'); % T = 6ms, B = 1.5kHz, bd = 300Hz
%RF = RF1.rf2 * 7; %refocs * 7; % 50 % 7
%load('/home/molin/shimm_nick/decompose_refocus/demo/ISMRM/compare/refocus_test/sagittal/pulsefixedpoint.mat')
load('/home/molin/shimm_nick/op_rewind/results/sagittal/pulse13rw.mat')
...
non_linear = gr(4:end, 1:500);
rewind = zeros(size(non_linear, 1), 250);
%rewind(:,1) = non_linear(:,500);
%rewind(:,:) = repmat(non_linear(:,500),1,250);
rewind = gr(4:end,501:750);
...
kk.lr(3,1:500) = 0.4;%0.8;%0.7; %2 * 4.8933;%7242 / 160; %%%%%% lr(2,:) or lr(3,:)
kk.lr(3,501:end) = -0.2; %-0.2;
%kk.lr(3,501:end) = -0.01;

kk.rw = rewind;
%kk.lr(2,1001:1500) = 0.001;
%kk.lr = gr(1:3,:);
kk.gr = [kk.lr; [kk.nr kk.rw]]; %gr + 5 * rand(size(gr));%
...
function pAD = OV90(cube, cube1, cube2, target, pIni)
  ...
  %plot_res(pIni, pIni, cube2, target, 'xy');
  %suptitle('OV90');
end

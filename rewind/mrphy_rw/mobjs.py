...
from mrphy_rw import γH, dt0, gmax0, smax0, rfmax0, T1G, T2G, π
from mrphy_rw import utils, beffective, sims
...
class Pulse(object):
    ...
    __slots__ = set(_readonly + _limits + ('rf', 'gr','lr','nr', 'rw', 'dt', 'desc'))

    def __init__(
            self,
            rf: Optional[Tensor] = None, gr: Optional[Tensor] = None, lr: Optional[Tensor] = None, nr: Optional[Tensor] = None, rw: Optional[Tensor] = None,
            dt: Tensor = dt0,
        ...
        self.lr, self.nr, self.rw = lr.to(**kw), nr.to(**kw), rw.to(**kw)
    ...
    def beff(
            ...
            γ: Tensor = γH, offset_loc_: Optional[Tensor] = None) -> Tensor:
        ...
        return beffective.rfgr2beff(mask, self.rf, self.gr, loc,
                                    Δf=Δf, b1Map=b1Map, γ=γ, offset_loc_ = offset_loc_)
    ...
    def to(self, device: torch.device = torch.device('cpu'),
        ...
        if (self.device != device) or (self.dtype != dtype):
            return Pulse(self.rf, self.gr, self.lr, self.nr, self.rw, dt=self.dt, desc=self.desc,
...
class SpinArray(object):
    ...
    def applypulse(
            ...
            b1Map: Optional[Tensor] = None, b1Map_: Optional[Tensor] = None, offset_loc_: Optional[Tensor] = None
            ) -> Tensor:
        ...
        beff_ = self.pulse2beff(pulse, loc_=loc_,
                                Δf_=Δf_, b1Map_=b1Map_, doEmbed=False, offset_loc_ = offset_loc_)
        #print(loc_)
        #print(beff_)
    ...
    def pulse2beff(
        ...
        beff_ = pulse.beff(self.mask, loc_, γ=self.γ_, Δf=Δf_, b1Map=b1Map_, offset_loc_ = offset_loc_)
...
class SpinCube(object):
    ...
    def applypulse(
        ...
        return self.spinarray.applypulse(pulse, doEmbed=doEmbed,
                                         ...
                                         loc=self.loc, b1Map_=b1Map_, offset_loc_ = offset_loc_)
    ...
    def pulse2beff(
        ...
        return self.spinarray.pulse2beff(pulse, self.loc_, doEmbed=doEmbed,
                                         Δf_=self.Δf_,
                                         b1Map=b1Map, b1Map_=b1Map_, offset_loc_ = offset_loc_)
